...
def grade(model_path, x_test, y_test):
  ...
  print('准确率为：', round(accuracy_score(model.is_malicious(x_test), y_test) * 100, 2))
  print('精确率为：', round(precision_score(model.is_malicious(x_test), y_test) * 100, 2))
  print('召回率为：', round(recall_score(model.is_malicious(x_test), y_test) * 100, 2))
  print('F1 score为：', round(f1_score(model.is_malicious(x_test), y_test) * 100, 2))
...
if __name__ == '__main__':
  ...
  target_features = traindf["is_mal"]
  all_features = traindf.drop(["is_mal"], axis=1)
  x_train, x_test, y_train, y_test = train_test_split(
    all_features, target_features, test_size=0.2, random_state=42
  )
  dump_path = "./model"
  # select_feature(all_features, target_features)
