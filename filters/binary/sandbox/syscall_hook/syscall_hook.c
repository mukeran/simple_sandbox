...
typedef asmlinkage long (*old_execve_t)(const char *__path, char *const __argv[], char *const __envp[]);
old_execve_t old_execve = NULL;

// Hook execve syscall
asmlinkage long hooked_execve(const char *__path, char *const __argv[], char *const __envp[]) {
    printk("Path: %s\n", __path);
...
static int hooked_init(void) {
    ...
    _system_call_off = (system_call_gate->offset_low & 0xffff) | (system_call_gate->offset_middle & 0xffff0000);
    ...
    old_execve = (old_execve_t) _sys_call_table[__NR_execve];
